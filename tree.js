async function buildTree(jwt, familyId) {
  // extend javascript array class by a remove function
  // copied from https://stackoverflow.com/a/3955096/12267732
  const res = await fetch(`https://spadok.herokuapp.com/api/tree/${familyId}/display`, {
    method: 'GET',
    headers: {
  ...
  function getParents(node) {
    var parents = [];
    if (node.data.isUnion) {
      node.data.partner.forEach((p_id) => parents.push(all_nodes.find((n) => n.id == p_id)));
    } else {
  ...
  function getOtherPartner(node, union_data) {
    var partner_id = union_data.partner.find((p_id) => (p_id != node.id) & (p_id != undefined));
  ...
  function getPartners(node) {
    ...
    if (node.data.isUnion) {
      node.data.partner.forEach((p_id) => partners.push(all_nodes.find((n) => n.id == p_id)));
    }
  ...
  function getOwnUnions(node) {
    if (node.data.isUnion) return [];
    unions = [];
    node.data.own_unions.forEach((u_id) => unions.push(all_nodes.find((n) => n.id == u_id)));
...
async function login() {
  ...
  // const email = 'morar.nikita@gmail.com';
  // const password = 'test1234';

  const res = await fetch('https://spadok.herokuapp.com/api/auth', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json;charset=utf-8',
...
function renderLoginForm() {
  document.body.innerHTML = `
    <div align="center" class="mt-3"><h1>&nbsp;</h1></div>
    <div align="center" class="login-field mt-5">

    <h2>Welcome to the Spadok Family Tree Viewer 🌳</h1>
    <h5>It is a part of <a target="_blank" rel="noopener noreferrer" href="https://spadok.herokuapp.com/swagger-ui.html">Spadok Project</a>. Mykyta Morar's Bachelor Thesis.</h5>

    <h3 class="mt-5">Please use your credentials to log in and see your family tree</h3>

      <div class="input-group mb-3 w-25 mt-4">
        <div class="input-group-prepend">
          <span class="input-group-text" id="basic-addon1">Email</span>
        </div>
        <input type="text" class="email-input form-control" placeholder="Your email" aria-label="Username" aria-describedby="basic-addon1">
      </div>
      ...
      <div class="input-group mb-3 w-25">
        ...
        <input type="password" class="password-input form-control" placeholder="Your password" aria-label="Password" aria-describedby="basic-addon1">
      </div>

      <button type="button" class="btn btn-success login-button">Log in</button>
    </div>
  `;
}
